...
<body>
<?php if($type == 4) : ?>
                        <button onclick="window.location='formulaire.php'"class="btn" id="user-btn"><span lang='en'>Adhesion formular</span><span lang='fr'>Formulaire d'adhésion</span><span lang='ar'></span></button>
    ...
    <header class="hero-section">
        <div class="content">
            ...
            <p class="sub-heading"><span lang='en'>Quality,Mastery and easy practice of sports</span><span lang='fr'>Qualité, Maîtrise et pratique facile du sport</span><span lang='ar'>الجودة والإتقان وسهولة ممارسة الرياضة</span></p>
        </div>

    </header>
    <!-- cards-container -->
    <?php 
    $total = "SELECT product, SUM(`quantity`) AS total FROM `sales` WHERE exists (select * from products where `name`= product) AND`state`='3' GROUP BY product  ORDER BY total DESC LIMIT 5";
$statement = $conn->query($total);
if (!$statement = $conn->query($total)) {
    echo 'Erreur';
} else {
    echo "<section class='product'>
    <h2 class='product-category'><span lang='en'>Best Sellings</span><span lang='fr'>Meilleures ventes</span><span lang='ar'>الأكثر مبيعا</span></h2>
    <button class='pre-btn'><img src='img/arrow.png' alt=''></button>
    <button class='nxt-btn'><img src='img/arrow.png' alt=''></button>
    <div class='product-container'>";
    foreach($statement as $row3){
        $theproduct = $row3['product'];
        $checking_product =   mysqli_query($conn,"SELECT * FROM products WHERE `name`='$theproduct'");
         $productdata = mysqli_fetch_array($checking_product);
...
document.querySelector('.star2-". $productdata['id'] ."').addEventListener('mouseover', function(){
    ...
    document.querySelector('.star3-". $productdata['id'] ."').addEventListener('mouseover', function(){
        ...
        document.querySelector('.star4-". $productdata['id'] ."').addEventListener('mouseover', function(){
            ...
            <form method='post' action='action/wishlist.php'>
        ...
        <div class='product-card'>
            ...
            <img src='img/icons/". $s5 ."' class='star'>
                ...
                <form target='_blank' method='post' action='action/wishlist.php'>
                <input type='hidden' name='username' value='". $name ."'>
                <input type='hidden'  name='name' value='". $productdata['name'] ."'>
                <input type='hidden' name='description' value='". $productdata['description_en'] ."'>
                <input type='hidden' name='pic' value='". $productdata['pic'] ."'>
...
?>
    ...
    <section class="collection-container">
        <a href="product.php?category=women" class="collection">
            <video src="img/woman.mp4" type="video/mp4" loop class="clip" muted></video>
            <p class="collection-title title-women"><span lang='en'>women's</span><span lang='fr'>femmes</span><span lang='ar'>نساء</span></p>
        </a>
        <a href="product.php?category=men" class="collection">
        <video src="img/man.mp4" type="video/mp4" loop class="clip" muted></video>
            <p class="collection-title title-men"><span lang='en'>men's</span><span lang='fr'>hommes</span><span lang='ar'>رجال</span></p>
        </a>
    </section>
    <script>
      ...
      <?php 
$sql2 = "SELECT * FROM products WHERE `type`='Shoes'  ORDER BY id DESC";
...
document.querySelector('.star2-". $productdata['id'] ."').addEventListener('mouseover', function(){
 ...
 <?php 
$sql2 = "SELECT * FROM products WHERE `type`='Shirts'  ORDER BY id DESC";
$result2 = $conn-> query($sql2);
if ($result2-> num_rows > 0) {
    echo "     <!-- cards-container -->
    <section class='product'>
